...
from src.data.collate_fns.Xray_collate_fn import Xray_collate_fn
from src.data.datasets.CustomXrayDataset import CustomXrayDataset
...
class XrayDataModule(BaseDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        # Load datasets
        raw_data_path = self.config["data"]["raw_data_path"] #data_dir(train)
        test_data_path = self.config["data"]["test_data_path"]#data_dir(test)
        train_df_path = self.config["data"]["train_df_path"]
        test_df_path = self.config["data"]["test_df_path"]
        self.train_info = pd.read_csv(train_df_path) #info_df(train)
        self.test_info = pd.read_csv(test_df_path) #info_df(test)

        self.train_dataset = CustomXrayDataset(
            data_dir=raw_data_path, info_df=self.train_info, is_inference=False, transform=train_transforms
        )

        self.test_dataset = CustomXrayDataset(
